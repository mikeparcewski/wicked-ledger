...
> **Provenance.** These modules were extracted verbatim from `wicked-testing`'s
> self-contained ledger lib (Stage 1 of the evidence-ledger → infra carve). The
> internals are intentionally byte-for-byte unchanged from their origin —
> including cosmetic details like the on-disk db filename (`wicked-testing.db`)
> and `[wicked-testing]` stderr prefixes. Rewiring the consumers
> (`wicked-testing` / `wicked-crew` / `wicked-garden`) onto this package is a
> separate, later stage; this release is **additive and non-breaking**.
