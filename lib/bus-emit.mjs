 ...
 * evidence_path, qe_version} + optional {verdict_id,
 * vault_payload_sha, artifact_count}, each null when the emitting site lacks it.
 * (`qe_version` was `wicked_testing_version` before the Phase 6c retirement;
 * the domain stamp was `wicked-testing`.)
 *
 ...
 * the single source of truth for the qe test-lifecycle event surface.
 */
...
export function emitBusEvent(type, payload = {}) {
  ...
  try {
    const res = spawnSync(
      "wicked-bus",
      ["emit", "--type", type, "--domain", "qe", "--payload", JSON.stringify(body)],
 ...
 * @param {string} qeVersion  QE toolchain version stamped into payloads
 *   (payload key `qe_version`; was `wicked_testing_version` pre-6c)
 ...
 */
export function domainEventToBusEvent(action, source, record, qeVersion) {
  if (!record) return null;
  const common = {
    project_id: record.project_id,
    qe_version: qeVersion,
  };
